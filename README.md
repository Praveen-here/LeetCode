...
└─────────────────────────────────────────────────────────┘
   ...
   ├─ Checking validity? (True/False answer)
   │  └─ Can I use constraints to validate?
   │     ├─ YES → GREEDY or SIMULATION
   │     └─ NO → BRUTE FORCE or MATH
   │
   ├─ Finding something? (One element, pair, etc.)
   │  └─ Sorted data available?
   │     ├─ YES → BINARY SEARCH
   │     └─ NO → HASH MAP or LINEAR SCAN
   │
   ├─ Counting something? (Frequency, combinations, etc.)
   │  └─ Need exact count or just presence?
   │     ├─ Frequency → HASH MAP
   │     └─ Presence → SET or HASH MAP
   │
   ├─ Optimizing? (Maximum, minimum, best path, etc.)
   │  └─ Can I use previous solutions?
   │     ├─ YES → DYNAMIC PROGRAMMING
   │     └─ NO → GREEDY or TWO POINTERS
   │
   ├─ Traversing structure? (Trees, graphs, lists)
   │  └─ What structure?
   │     ├─ Linear → TWO POINTERS or SLIDING WINDOW
   │     ├─ Tree → DFS/BFS or RECURSION
   │     └─ Graph → DFS/BFS or GRAPH ALGORITHMS
   │
   └─ Mathematical formula? (Calculate something)
      └─ Do I need to explore values?
         ├─ NO → DIRECT FORMULA or MATH
